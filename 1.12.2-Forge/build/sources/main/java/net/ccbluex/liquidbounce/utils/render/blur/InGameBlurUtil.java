...
public class InGameBlurUtil {
    ...
    public static void renderGaussianBlur(final float radius, final float compression, final boolean callEvent, final boolean backTrackBlurOnScreen) {
        ...
        BlurSettings.stuffToBlur(false);
        BlurSettings.stuffToBlur2(false);
    ...
    public static void renderGaussianBlur2(final float radius, final float compression, final boolean callEvent, final boolean backTrackBlurOnScreen) {
        sr = new ScaledResolution(mc);
        GlStateManager.enableBlend();
        GlStateManager.color(1, 1, 1, 1);
        OpenGlHelper.glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE, GL_ZERO);
        ...
        RenderUtils.drawRoundedRectfix(middleScreen - 91, sr.getScaledHeight() - 24, 182 , sr.getScaledHeight(),0F,-1);

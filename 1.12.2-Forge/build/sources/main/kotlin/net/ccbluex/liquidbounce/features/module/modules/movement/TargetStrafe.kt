...
class TargetStrafe : Module() {
    ...
    val onGroundValue = BoolValue("OnlyOnGround",false)
    val holdSpaceValue = BoolValue("HoldSpace", false)
    val onlySpeedValue = BoolValue("OnlySpeed", false)
    val onlyPitSpeedValue = BoolValue("OnlyPitSpeed", false)
    ...
    fun onRender3D(event: Render3DEvent) {
        if(mc.thePlayer!!.onGround && onGroundValue.get() || !onGroundValue.get()) {
            ...
            if (renderModeValue.get() != "None" && canStrafe(target)) {
                ...
                if (renderModeValue.get().equals("Circle", ignoreCase = true)) {
                    ...
                    for (i in 0..10) {
                        ...
                        if (rad < 1.5 && rad > 0.7) {
                            counter[0] = counter[0] + 1
                            RenderUtils.glColor4(ColorManager.astolfoRainbow(counter[0] * 100, 5, 107))
                            GL11.glVertex3d(

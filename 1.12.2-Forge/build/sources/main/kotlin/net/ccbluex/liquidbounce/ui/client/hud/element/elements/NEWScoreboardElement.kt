 ...
 */
@ElementInfo(name = "NEWScoreboardElement")//by Angst
class NEWScoreboardElement(
        x: Double = 5.0,
        y: Double = 0.0,
        scale: Float = 1F,
    ...
    private val OutmodeValue = ListValue("LOL", arrayOf("TxSense"), "TxSense")
    ...
    private val noshadow = BoolValue("NoShadow",true)
    ...
    private val fontValue = FontValue("Font", Fonts.font25)
    ...
    override fun drawElement(): Border? {
        ...
        if (OutmodeValue.get().equals("TxSense", true)) {
            GL11.glTranslated(-renderX, -renderY, 0.0)
            maxHeight + fontRenderer.fontHeight + 10f
            RoundedUtil.drawRound(renderX.toFloat() + l1 - 7f,
                    renderY.toFloat() - 5f,
                    -l1 + 16f,
                    maxHeight + fontRenderer.fontHeight + 10f,
                    radius.get().toFloat(),Color(0, 0, 0, this.bgValue.get()))
            GL11.glTranslated(renderX, renderY, 0.0)
        }
        if (outline.get()){

...
object ColorUtils {
    ...
    fun interpolateColorsBackAndForth(speed: Int, index: Int, start: Color?, end: Color?, trueColor: Boolean): Color? {
        ...
        return if (trueColor) RenderUtils.interpolateColorHue(
            start,
            end,
            angle / 360f
        ) else RenderUtils.interpolateColorC(
            start,
            end,
            angle / 360f
        )
    ...
    fun ALLColor(offset: Long): Color {
        val currentColor = Color(
            Color.HSBtoRGB(
                (Minecraft.getMinecraft().player.ticksExisted / 50.0 + Math.sin(50 / 50.0 * 1.6) % 1).toFloat(),
                0.4F,
                0.9F
            )
        )
    ...
    fun TwoRainbow(offset: Long, alpha: Float): Color {
        ...
        return Color(
            currentColor.getRed() / 255.0F * 1.0F,

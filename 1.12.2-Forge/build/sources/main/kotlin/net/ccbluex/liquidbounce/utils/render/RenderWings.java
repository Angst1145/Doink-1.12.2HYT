...
public class RenderWings extends ModelBase {
    ...
    public RenderWings() {
        if(WingMode.equals("Dragon")) { this.location = new ResourceLocation("tomk/wings/DragonWings.png"); } else if(WingMode.equals("Simple")) { this.location = new ResourceLocation("tomk/wings/NeonWings.png"); }
    ...
    public void renderWings(float partialTicks) {
        double rotate;
        if (RotationUtils.targetRotation != null && rotations.getBodyValue().get()) {
            rotate = this.interpolate(mc.player.rotationYawHead, mc.player.renderYawOffset, partialTicks);
        } else {
            rotate = this.interpolate(mc.player.prevRenderYawOffset, mc.player.renderYawOffset, partialTicks);
        }
        if (mc.player.isRiding()) rotate = this.interpolate(mc.player.prevRenderYawOffset, mc.player.renderYawOffset, partialTicks);
        ...
        if (Wings.getColourType().get().equals("Chroma")) {
            RenderUtils.glColor(ColorUtils.rainbow(), 255);
        } else if (Wings.getColourType().get().equals("Custom")) {
            RenderUtils.glColor(new Color(Wings.getCR().get(), Wings.getCG().get(), Wings.getCB().get()), 255);
        } else {

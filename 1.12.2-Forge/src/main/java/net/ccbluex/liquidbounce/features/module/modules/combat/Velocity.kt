...
class Velocity : Module() {
    ...
    private val newaac4XZReducerValue =
            FloatValue("NewAAC4XZReducer", 0.45F, 0F, 1F).displayable { modeValue.get().toLowerCase().equals("aac4") }

    private val velocityTickValue = IntegerValue("VelocityTick", 1, 0, 10)

    // Reverse
    private val reverseStrengthValue =
            FloatValue("ReverseStrength", 1F, 0.1F, 1F).displayable { modeValue.get().toLowerCase().equals("reverse") }
    private val reverse2StrengthValue = FloatValue("SmoothReverseStrength", 0.05F, 0.02F, 0.1F).displayable {
        modeValue.get().toLowerCase().equals("smoothreverse")
    }

    private val hytpacketaset = FloatValue("HytPacketASet", 0.35F, 0.1F, 1F).displayable {
        modeValue.get().toLowerCase().equals("hytpacketa") || modeValue.get().toLowerCase().equals("hytpacketfix")
    }
    private val hytpacketbset =
            FloatValue("HytPacketBSet", 0.5F, 1F, 1F).displayable { modeValue.get().toLowerCase().equals("hytpacketa") }

    // AAC Push
    private val aacPushXZReducerValue =
            FloatValue("AACPushXZReducer", 2F, 1F, 3F).displayable { modeValue.get().toLowerCase().equals("aac4push") }
    private val aacPushYReducerValue =
            BoolValue("AACPushYReducer", true).displayable { modeValue.get().toLowerCase().equals("aac4push") }
    ...
    private val grimReduceNoMotionY =
            BoolValue("GrimNoMotionY", true).displayable { modeValue.get().toLowerCase().equals("grimreduce") }


    //Custom
    private val customX =
            FloatValue("CustomX", 0F, 0F, 1F).displayable { modeValue.get().toLowerCase().equals("custom") }
    private val customYStart =
            BoolValue("CanCustomY", false).displayable { modeValue.get().toLowerCase().equals("custom") }
    private val customY =
            FloatValue("CustomY", 1F, 1F, 2F).displayable { modeValue.get().toLowerCase().equals("custom") }
    private val customZ =
            FloatValue("CustomZ", 0F, 0F, 1F).displayable { modeValue.get().toLowerCase().equals("custom") }
    private val customC06FakeLag =
            BoolValue("CustomC06FakeLag", false).displayable { modeValue.get().toLowerCase().equals("custom") }
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (thePlayer.isInWater || thePlayer.isInLava || thePlayer.isInWeb)
            return
        if (thePlayer.isDead || (noFireValue.get() && mc.thePlayer!!.burning) || (cobwebValue.get() && mc.thePlayer!!.isInWeb) || (onlyGroundValue.get() && !mc.thePlayer!!.onGround)) return
        when (modeValue.get().toLowerCase()) {
            ...
            "grimreduce" -> if (thePlayer.hurtTime > 0) {
                thePlayer.motionX += -1.0E-7
                if (!grimReduceNoMotionY.get()) {
            ...
            "custom" -> {
                if (thePlayer.hurtTime > 0 && !thePlayer.isDead && !mc.thePlayer!!.isPotionActive(
                    ...
                    if (customC06FakeLag.get()) mc.netHandler.addToSendQueue(

...
@ModuleInfo(name = "KillAura_Angst", description = "Automatically attacks targets around you.",
        category = ModuleCategory.DONK, keyBind = Keyboard.KEY_R)
class KillAuraTest : Module() {
    ...
    val falldownValue = BoolValue("FallDownFix",true)
    val falldownrangeValue = FloatValue("FallDownRange",3.2f,1f,8f)
    val groundrangeValue = FloatValue("GroundRange",3.6F,1f,8f)
    ...
    val spoofmultiValue = BoolValue("SpoofMulti",true)
    ...
    // AutoBlock
    private val autoBlockValue = ListValue("AutoBlock", arrayOf("Off", "Packet", "AfterTick","GameSettings"), "Packet")
    ...
    private val rotations = ListValue("RotationMode", arrayOf(
            "None",
            "ArtDayAlpha-Ground",
            "ArtDayAlpha-FallDown"), "ArtDayAlpha-Ground")
    val strafeFixValue = BoolValue("StrafeFix",true)
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        //spoofmulti
        if(spoofmultiValue.get()){
            targetModeValue.set("Switch")
            switchDelayValue.set(-1)
        }
        //rangefix
        if(falldownValue.get() && mc.thePlayer!!.fallDistance > 0 && rangeValue != falldownValue) rangeValue.set(falldownrangeValue.get())
        else rangeValue.set(groundrangeValue.get())
        //rotationfix
        if(falldownValue.get() && mc.thePlayer!!.fallDistance > 0 && rotations.get() != "ArtDayAlpha-FallDown") rotations.set("ArtDayAlpha-FallDown")
        else rotations.set("ArtDayAlpha-Ground")
    ...
    private fun attackEntity(entity: IEntityLivingBase) {
        ...
        if(autoBlockValue.get().equals("GameSettings", true)) {
    ...
    private fun updateRotations(entity: IEntity): Boolean {
        ...
        if (rotations.get().equals("ArtDayAlpha-Ground", ignoreCase = true)) {
            if (predictValue.get())
        ...
        if (rotations.get().equals("ArtDayAlpha-FallDown", ignoreCase = true)) {
            if (predictValue.get())
                boundingBox = boundingBox.offset(
                        (entity.posX - entity.prevPosX) * RandomUtils.nextFloat(minPredictSize.get(), maxPredictSize.get()),
                        (entity.posY - entity.prevPosY) * RandomUtils.nextFloat(minPredictSize.get(), maxPredictSize.get()),

...
class Fly : Module() {
    ...
    private val canClipValue = BoolValue("vulcanclipCanClip", true)
...
//                    mc.effectRenderer.spawnEffectParticle(EnumParticleTypes.FLAME.getParticleID(), thePlayer.posX, thePlayer.posY + 0.2, thePlayer.posZ, -thePlayer.motionX, -0.5, -thePlayer.motionZ)
    ...
    fun onMotion(event: MotionEvent) {
        if (modeValue.get().equals("vulcanclip", ignoreCase = true)) {
    ...
    fun onPacket(event: PacketEvent) {
        if (modeValue.get().equals("vulcanclip", ignoreCase = true)) {
            ...
            val thePlayer = mc.thePlayer!!
            if(event.packet is SPacketPlayerPosLook && waitFlag) {
                waitFlag = false
                thePlayer.setPosition(thePlayer.posX, thePlayer.posY, thePlayer.posZ)
                mc.netHandler.addToSendQueue(classProvider.createCPacketPlayerPosLook(mc.thePlayer!!.posX, mc.thePlayer!!.posY, mc.thePlayer!!.posZ, mc.thePlayer!!.rotationYaw, mc.thePlayer!!.rotationPitch, false))
                event.cancelEvent()
                mc.thePlayer!!.jump()
                clip(0.127318f, 0f)
                clip(3.425559f, 3.7f)

...
public class GlowESPHelper {
    ...
    public static boolean isInView(Entity ent) {
        frustum.setPosition(mc.getRenderViewEntity().getPosX(), mc.getRenderViewEntity().getPosY(), mc.getRenderViewEntity().getPosZ());
    ...
    public static Vector3f projectOn2D(float x, float y, float z, int scaleFactor) {
        ...
        if (GLU.gluProject(x, y, z, floatBuffer1, floatBuffer2, intBuffer, windPos)) {
            return new Vector3f(windPos.get(0) / scaleFactor, (mc2.displayHeight - windPos.get(1)) / scaleFactor, windPos.get(2));
        }
        return null;
    }

    public static double[] getInterpolatedPos(Entity entity) {
        float ticks = mc2.timer.renderPartialTicks;
        return new double[]{
                MathUtils.interpolate(entity.lastTickPosX, entity.posX, ticks) - mc.getRenderManager().getRenderPosX(),
                MathUtils.interpolate(entity.lastTickPosY, entity.posY, ticks) - mc.getRenderManager().getRenderPosY(),
                MathUtils.interpolate(entity.lastTickPosZ, entity.posZ, ticks) - mc.getRenderManager().getRenderPosZ()
        };
    }

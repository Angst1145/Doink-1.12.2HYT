...
abstract class Element(var x: Double = 2.0, var y: Double = 2.0, scale: Float = 1F,
    ...
    open fun drawBoarderBlur(blurRadius: Float = blurValue.get()) {
        ...
        tomk.utils.BlurUtils.draw(posX * scale, posY * scale, width * scale, height * scale, blurRadius)
    }
    protected fun blur(x: Float, y: Float, x2: Float, y2: Float) {
        ...
        tomk.utils.BlurUtils.draw((renderX + (x.coerceAtMost(x2))).toFloat() * scale, (renderY + (y.coerceAtMost(y2))).toFloat() * scale, abs(x2 - x) * scale, abs(y2 - y) * scale, blurValue.get())
    }
    /**
...
annotation class ElementInfo(val name: String, val single: Boolean = false, val force: Boolean = false, val disableScale: Boolean = false, val priority: Int = 0, val blur: Boolean = false)

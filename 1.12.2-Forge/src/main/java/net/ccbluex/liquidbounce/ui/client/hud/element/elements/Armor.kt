...
class Armor(x: Double = -8.0, y: Double = 57.0, scale: Float = 1F,
    ...
    override fun drawElement(): Border {
        ...
        //Rect
        RenderUtils.drawRoundedRect(0F, 0F, 50F, 76F,radiusValue.get(), Color(11,11,12,this.alphaValue.get()).rgb )
        ...
        if (bV.get()) {
            GL11.glTranslated(-renderX, -renderY, 0.0)
            GL11.glPushMatrix()
            tomk.utils.BlurBuffer.CustomBlurRoundArea(renderX.toFloat(), renderY.toFloat()  , 50F, 76F,radiusValue.get(),BlurStrength.get())
            GL11.glPopMatrix()
            GL11.glTranslated(renderX, renderY, 0.0)
        }

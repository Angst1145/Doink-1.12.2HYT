...
import tomk.utils.blur.BlurBuffer
...
class Text(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F,
    ...
    val r = IntegerValue("Top-bg-Red", 0, 0, 255)
    val g = IntegerValue("Top-bg-Green", 0, 0, 255)
    val b = IntegerValue("Top-bg-Blue", 0, 0, 255)
    val alpha = IntegerValue("Top-BG-Alpha", 100, 0, 255)
    private val blurValuee = BoolValue("Top-Blur", true)
    private val BlurStrength = FloatValue("BlurStrength", 5f,0f,20f)
    private val shadowValuee = FloatValue("ShadowStrength", 10f,0f,20f)
    ...
    private val display: String
        get() {
            val textContent = if (displayString.get().isEmpty() && !editMode)
                    ...
                    LiquidBounce.CLIENT_NAME + " |" +  "Fps:%fps% "
...
//            "worldTime" -> return SessionUtils.getFormatWorldTime()
    ...
    override fun drawElement(): Border? {
        ...
        val hud = LiquidBounce.moduleManager[HUD::class.java] as HUD
        val mwc1 = Color(AColorPalette.r.get(),AColorPalette.g.get(),AColorPalette.b.get()).rgb
        val mwc2 = Color(AColorPalette.r2.get(),AColorPalette.g2.get(),AColorPalette.b2.get()).rgb
        if (this.Mode.get().equals("Top")) {
            when (shadowValue.get()) {
                ...
                "Shadow" -> {
                    ...
                    GL11.glPushMatrix()
                    ShadowUtils.shadow(shadowValuee.get(),{
                        GL11.glPushMatrix()
                        GL11.glTranslated(renderX, renderY, 0.0)
                        GL11.glScalef(scale, scale, scale)
                        RenderUtils.drawRoundedRect2(-2f, -2f, (fontRenderer.getStringWidth(displayText) + 2).toFloat(), fontRenderer.fontHeight.toFloat(), 0f, Color(0, 0, 0).rgb)
                        GL11.glPopMatrix()
            ...
            if (blurValuee.get()){
                GL11.glTranslated(-renderX, -renderY, 0.0)
                GL11.glPushMatrix()
                BlurBuffer.blurRoundArea(renderX.toFloat()-0.5f, renderY.toFloat(), (fontRenderer.getStringWidth(displayText) + 2).toFloat()+1, fontRenderer.fontHeight.toFloat()
                    , 0)
                GL11.glPopMatrix()
                GL11.glTranslated(renderX, renderY, 0.0)
            }
       ...
       if (this.Mode.get().equals("Onetap")) {
           RoundedUtil.drawRound(-2.8f,-3f, fontRenderer.getStringWidth(displayText).toFloat()+0.91f, fontRenderer.fontHeight.toFloat()+4.12f,radiusValue.get()+0.5f, Color(r.get(), g.get(), b.get(), alpha.get()))
            //RenderUtils.drawGradientSideways(-1.0,-2.0, (fontRenderer.getStringWidth(displayText) + 1).toDouble(), -4.0, mwc1, mwc2)
           GL11.glTranslated(-renderX, -renderY, 0.0)
           GL11.glScalef( 1F,  1F,  1F)
           GL11.glPushMatrix()
           ...
           ShadowUtils.shadow(shadowValuee.get(),{
               ...
               GL11.glTranslated(renderX, renderY, 0.0)
               GL11.glScalef(scale, scale, scale)
               RenderUtils.drawRoundedRect2(-2.85f,-3f, fontRenderer.getStringWidth(displayText).toFloat()+0.91f, fontRenderer.fontHeight.toFloat()+4.12f, radiusValue.get()+0.3f, Color(0, 0, 0).rgb)
               GL11.glPopMatrix()
           ...
           if (blurValuee.get()){
               GL11.glTranslated(-renderX, -renderY, 0.0)
               GL11.glPushMatrix()
               BlurUtils.CustomBlurRoundArea(renderX.toFloat()-4.6f,renderY.toFloat()-3.8f, fontRenderer.getStringWidth(displayText).toFloat()+3.85f, fontRenderer.fontHeight.toFloat()+5.8f,radiusValue.get(), BlurStrength.get())
               GL11.glPopMatrix()
               GL11.glTranslated(renderX, renderY, 0.0)
           }
        }
